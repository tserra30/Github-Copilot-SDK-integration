...
For bridge networking changes, run the bridge and SDK client in **separate network namespaces** on a user-defined Docker network, connecting by the bridge's container hostname. Do not use shared host networking for both: it hides a loopback-only listener. Confirm the real startup script binds `0.0.0.0:8000` using `--host` (hidden from CLI 1.0.83 help), without published host ports, then exercise SDK connection, session cleanup, and MCP actions. Outside Supervisor, a test fixture may supply Bashio's add-on options; explicitly distinguish that from Supervisor lifecycle validation.

For authentication/logging changes, verify that startup performs no model-based authentication probe, reports a configured token without claiming it is valid, and preserves the CLI's warning for an unprotected bridge endpoint. Exercise genuine SDK authentication and model requests from another container. Inspect INFO/DEBUG integration milestones and failure paths for accidental token, MCP-header, prompt, response, or raw exception disclosure. A failed request must not produce a success/completion message.
